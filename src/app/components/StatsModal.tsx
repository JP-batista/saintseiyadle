...
export default function StatsModal({ isOpen, onClose }: StatsModalProps) {
  ...
  // Prepara dados para o gráfico de linha (últimos 30 jogos vencidos)
  ...
  // Histórico de jogos
  const wonGames = gamesHistory.filter(g => g.won);
  const displayedHistory = showAllHistory ? wonGames : wonGames.slice(0, 5);
  const hasMoreGames = wonGames.length > 5;

  return (
    <div 
      // FUNDO DO MODAL: Trocado bg-opacity-75 por /60 e adicionado backdrop-blur
      className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4"
      onClick={onClose}
    >
      <div 
        // CONTAINER PRINCIPAL: Trocado bg-gray-800 por backdrop-gradient
        className="
          ...
          overflow-y-auto custom-scrollbar flex flex-col /* Adicionado flex-col */
        "
      ...
      >
        ...
        {/* Content - flex-grow para empurrar o footer para baixo */}
        <div className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6 flex-grow">
          
          {/* Estatísticas principais */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-3 md:gap-4">
            {/* CARD DE STAT: Trocado bg-gray-700 por painel de vidro e adicionado hover-lift */}
            <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-3 sm:p-4 text-center transition-all duration-300 hover:bg-gray-900/70 hover:border-gray-600/50 hover:scale-105">
              <div className="text-3xl sm:text-4xl font-bold text-green-400">{totalWins}</div>
              <div className="text-sm sm:text-base text-gray-300 mt-1 uppercase tracking-wider font-semibold">Vitórias</div>
            </div>

            <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-3 sm:p-4 text-center transition-all duration-300 hover:bg-gray-900/70 hover:border-gray-600/50 hover:scale-105">
              <div className="text-3xl sm:text-4xl font-bold text-blue-400">{averageAttempts}</div>
              <div className="text-sm sm:text-base text-gray-300 mt-1 uppercase tracking-wider font-semibold">Média</div>
            </div>

            <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-3 sm:p-4 text-center transition-all duration-300 hover:bg-gray-900/70 hover:border-gray-600/50 hover:scale-105">
              <div className="text-3xl sm:text-4xl font-bold text-purple-400">{firstTryWins}</div>
              <div className="text-sm sm:text-base text-gray-300 mt-1 uppercase tracking-wider font-semibold">1ª Tentativa</div>
            </div>

            <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-3 sm:p-4 text-center transition-all duration-300 hover:bg-gray-900/70 hover:border-gray-600/50 hover:scale-105">
              <div className="text-3xl sm:text-4xl font-bold text-orange-400">{currentStreak}</div>
              <div className="text-sm sm:text-base text-gray-300 mt-1 uppercase tracking-wider font-semibold">Sequência</div>
            </div>
            ...
            <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-3 sm:p-4 text-center transition-all duration-300 hover:bg-gray-900/70 hover:border-gray-600/50 hover:scale-105 col-span-2 sm:col-span-1">
              <div className="text-3xl sm:text-4xl font-bold text-yellow-400">{maxStreak}</div>
              <div className="text-sm sm:text-base text-gray-300 mt-1 uppercase tracking-wider font-semibold">Recorde</div>
            </div>
          </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                    ...
                    <Line 
                      ...
                      strokeWidth={2}
                      dot={{ fill: '#FCD34D', r: 3 }}
                      activeDot={{ r: 5, fill: '#FCD34D', stroke: 'rgba(252, 211, 77, 0.5)', strokeWidth: 4 }}
                    />
                  </LineChart>
                </ResponsiveContainer>
              </div>
          ...
          {displayedHistory.length > 0 && (
            // CONTAINER HISTÓRICO: Estilo de vidro
            <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-3 sm:p-4">
              <h3 className="text-base sm:text-lg md:text-xl font-bold text-yellow-400 mb-3 sm:mb-4 text-center animate-text-glow">
              ...
              {/* BOTÃO VER MAIS: Estilo de vidro secundário */}
          ...
          {/* ESTADO VAZIO: Estilo de vidro com borda tracejada */}
        ...
        <div className="
          ...
          p-3 sm:p-4 z-20
        ">
          {/* BOTÃO FECHAR: Estilo gradiente e brilho */}
          <button
            onClick={onClose}
